...
func run() -> void:
	...
	for page in 4:
		...
		assert(rows.size()==([4,4,5,0][page]),"Every character move must be visible")
		if page<3:
	...
	press(main,KEY_LEFT);assert(main.moves_page==3,"First tab wraps to last")
	...
	print("Move list UI: four pages, key remapping, tab wrapping and pause return PASS")
