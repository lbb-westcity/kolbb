...
必杀只需依次按上下左右，无需斜方向。`下、前 + 拳` 发射飞行道具：面朝右时依次按 S、D，再按 J 或 U；朝左时把 D 换成 A。数字简写为 2 下、6 前、4 后、8 上。所有必杀、食物规则和 MAX 取消可在游戏“出招表”查看。出招表按角色分页，左右方向键切页，共通操作单独展示；按键提示跟随自定义键位。键位、音量、分辨率、震屏和减少闪光可在设置中调整。
